...
import { Table, message, Divider, Button, Form, Popconfirm, Input, Modal, Select } from 'antd';
...
const globalGetProductOptions = (data) => {
    let options = [];
          if(data){
            _.each(data,item => {
                options.push(
...
class EditableCell extends React.Component{
    getInput = () => {
            if (this.props.inputtype === 'input'){
                ...
                >
                {globalGetProductOptions(productStore.productType)}
                </Select>;
            }
...
class ProductName extends React.Component{
  constructor(props){
    super(props);
    let options = globalGetProductOptions(productStore.productType);
    this.state = {
      editingkey: '',
      showModal: false,
      inputName: '',
  ...
  save = (form, key) => {
    form.validateFields((error, row) => {
      ...
      const value = form.getFieldValue('productTypeId');
