...
const apiConfig = manage.product;
...
@observer
class ProductMgr extends React.Component{
  constructor(props){
    super(props);
    this.state = {
      editingkey: '',
  ...
  delete = (id) => {
    productAction.deleteProduct(id).then((flag) => {
      if(flag){
  ...
  save = (form, key) => {
    form.validateFields((error, row) => {
      ...
      if (index > -1){
        ...
        productAction.updateProduct(data).then((flag) => {
          if(flag){
  ...
  onCustomerAdd = (name,visible) => {
    ...
    productAction.addProduct(data).then((flag)=>{
      if(flag){
        this.setState({
          showModal: visible,
        });
  ...
  fetchData = async (pager) => {
    ...
    fetchApi(apiConfig.list, pager).then((res) => {
      ...
      if ((res.status === '0')){
        ...
        this.setState({
          data: res.data.productMgmtTypes,
          loading: false,
          pagination
      });
      } else {
  ...
  render(){
    ...
    return(
      <div className="customer">
        <Button type="primary" className="add-product-btn" onClick={this.onAddProductBtnClick}>新增产品类别</Button>
        ...
        >
          ...
          >
            ...
            <Col span={12}>
              <Input placeholder="产品类别" onChange={this.onInputChange}/>
            </Col>
          </Row>
        </Modal>
