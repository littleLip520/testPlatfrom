...
const apiConfig = manage.customer;
const { action: {customerAction} } = mobx;
...
class Customer extends React.Component{
  ...
  delete = (id) => {
    customerAction.deleteCustomer(id).then((flag) => {
      if(flag){
  ...
  save = (form, key) => {
    form.validateFields((error, row) => {
      ...
      if (index > -1){
        ...
        customerAction.updateCustomer(data).then((flag) => {
          if(flag){
  ...
  onCustomerAdd = (name,visible) => {
    customerAction.addCustomer(name).then((flag)=>{
      if(flag){
        this.setState({
          showModal: visible,
        });
  ...
  render(){
    ...
    return(
      <div className="customer">
        <Button type="primary" className="add-product-btn" onClick={this.onAddProductBtnClick}>新增客户源</Button>
        ...
        >
          ...
          >
            ...
            <Col span={12}>
              <Input placeholder="客户名称" onChange={this.onInputChange}/>
            </Col>
          </Row>
        </Modal>
