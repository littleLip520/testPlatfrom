...
const apiConfig = manage.pipeline;
const { action: {piplineAction} } = mobx;
...
class Pipeline extends React.Component{
  constructor(props){
    ...
    this.columns = [
      {
        title: '流水线节点',
        dataIndex: 'pipelineNodeName',
        width: '20%',
        key: 'pipelineNodeName',
        align: 'center',
        editable: true
      },
      {
  ...
  delete = (id) => {
    piplineAction.deletePipeline(id).then((flag) => {
      if(flag){
  ...
  save = (form, key) => {
    form.validateFields((error, row) => {
      ...
      if (index > -1){
        const data = {
          id: key,
          pipelineNodeName: row.pipelineNodeName,
        };
        piplineAction.updatePipeline(data).then((flag) => {
          if(flag){
  ...
  onCustomerAdd = (name,visible) => {
    piplineAction.addPipeline(name).then((flag)=>{
      if(flag){
        this.setState({
          showModal: visible,
        });
  ...
  render(){
    ...
    return(
      <div className="customer">
        <Button type="primary" className="add-product-btn" onClick={this.onAddProductBtnClick}>新增pipeline节点</Button>
        ...
        >
          ...
          >
            <Col span={6}>
              <span>pipeline节点：</span>
            </Col>
            <Col span={12}>
              <Input placeholder="pipeline节点" onChange={this.onInputChange}/>
            </Col>
          </Row>
        </Modal>
